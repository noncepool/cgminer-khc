...
extern bool kryptohash_prepare_work(struct thr_info __maybe_unused *thr, struct work *work);
#endif
...
char *set_shaders_mul(char *arg)
{
    ...
    nextptr = strtok(arg, ",");
    if (nextptr == NULL)
        return "Invalid parameters for set shaders multiplier";
    val = atoi(nextptr);

    gpus[device++].shaders_mul = val;
    ...
    if (device == 1) {
        for (i = device; i < MAX_GPUDEVICES; i++)
            gpus[i].shaders_mul = gpus[0].shaders_mul;
    }
...
#ifdef USE_KRYPTOHASH
static cl_int queue_kryptohash_kernel(_clState *clState, dev_blk_ctx *blk, __maybe_unused cl_uint threads)
{
    cl_kernel *kernel = &clState->kernel;
    unsigned int num = 0;
    cl_int status = 0;
    cl_ulong le_target = blk->target;
    unsigned int *nonce = (unsigned int *)(blk->kryptohash_data + 116);

    *nonce = blk->nonce;

    status = clEnqueueWriteBuffer(clState->commandQueue, clState->kryptohash_CLbuffer, true, 0, KRYPTOHASH_INBUFFER_SZ, (void *)&blk->kryptohash_data[0], 0, NULL, NULL);
    if (status != CL_SUCCESS)
    {
        applog(LOG_ERR, "Error %d: Enqueing input buffer", status);
        return status;
    }

    CL_SET_ARG(clState->kryptohash_CLbuffer);
    CL_SET_ARG(clState->outputBuffer);
    CL_SET_ARG(clState->scratchpad);
	CL_SET_ARG(le_target);
...
static void opencl_detect(bool hotplug)
{
	...
	/* If opt_g_threads is not set, use default 1 thread on scrypt/kryptohash and
	 * 2 for regular mining */
	if (opt_g_threads == -1) {
		if (opt_scrypt || opt_kryptohash)
			opt_g_threads = 1;
		else
			opt_g_threads = 2;
	}
...
#ifdef USE_KRYPTOHASH
    if (opt_kryptohash) {
        kryptohash_prepare_work(thr, work);
    }
    else
#endif
...
static int64_t opencl_scanhash(struct thr_info *thr, struct work *work,	int64_t __maybe_unused max_nonce)
{
	...
	if (clState->goffset) {
		...
		status = clEnqueueNDRangeKernel(clState->commandQueue, *kernel, 1, global_work_offset, globalThreads, localThreads, 0,  NULL, NULL);
	} 
	else
#else
    {
