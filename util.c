...
{
    uint32_t *hash40 = (uint32_t *)hash;
    uint32_t *target40 = (uint32_t *)target;
    bool rc = true;
    int i;

    for (i = 36 / 4; i >= 0; i--) {
        uint32_t h40tmp = le32toh(hash40[i]);
        uint32_t t40tmp = le32toh(target40[i]);
    ...
    if (opt_debug) {
        unsigned char hash_swap[40], target_swap[40];
        char *hash_str, *target_str;

        swab320(hash_swap, hash);
        swab320(target_swap, target);
        hash_str = bin2hex(hash_swap, 40);
        target_str = bin2hex(target_swap, 40);
